    ...
    def observe(self, s1, a, r, s2, isEnd):
        ...
        if isEnd:
            #   batch = random.sample(self.replayMem, self.batchSize)
              ...
              init_states, actions, rewards, next_states, isFinal = batch#zip(*batch)
              #init_states = np.concatenate(init_states)
              #next_states = np.concatenate(next_states)
              while True:
                targets = self.QNetwork.predict(init_states)
                print("Target estimate by network: {}".format(targets))
                estimated_values = self.TargetQNetwork.predict(next_states)
              
                targets[range(self.batchSize)] = [0,1] #+ (self.discountFactor*np.max(estimated_values, axis=1)*np.invert(isFinal))
              
                loss = self.QNetwork.train_on_batch(init_states, targets)
                print("Actual_Reward: {}\nLoss: {}".format(rewards, loss))
